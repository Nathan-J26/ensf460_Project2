...
{
    char buffer[6]; // enough for max 5 digits + null terminator
    int i = 0;
    ...
    while (Dec_num > 0 && i < 5)
    {
        buffer[i++] = (Dec_num % 10) + '0'; // extract last digit
        Dec_num /= 10;
    }
...
void clearTerminal() {
    // ANSI escape sequence: ESC[2J clears the screen, ESC[H moves cursor to top-left
